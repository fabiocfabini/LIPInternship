...
from models import LitVariationaAutoEncoder
...
if __name__ == "__main__":
    ...
    x_train, y_train, w_train = normalize(train)
    x_val, y_val, w_val = normalize(val)
    x_test, y_test, w_test = normalize(test)
    ...
    vae_test_dataset = AtlasDataset(x_test[y_test == BKG], y_test[y_test == BKG], w_test[y_test == BKG])
    ...
    vae = LitVariationaAutoEncoder(x_train.shape[1], 4, [64, 32, 16, 12], rec_loss="mse", lr_patience=10, rec_loss_weight=1e4)
